...
for num in range(0, 10):  # num이 10번 돌아감
    ...
    img = Image.open("./living_500.jpg") #이미지 오픈
    key_num = random.randint(1, 6)  # 뽑을 키워드 수 랜덤 생성
    selected_key = random.sample(new_key, key_num)  # new_key 리스트에서, 랜덤 수 만큼 키워드수 selected_key list에 담기
    print(selected_key)

    matrix500x500 = [[0 for col in range(500)] for row in range(500)]
    ...
    for i in range(0, key_num):  # key_num 만큼 글자를 생성해야하니까
        ...
        while True:
            ...
            fnt = ImageFont.truetype("../Library/Fonts/" + str(font) + ".otf", size)
            ...
            flag = True
            for txt_i in range(a[0], d[0] + 1):
                for txt_j in range(a[1], d[1] + 1):
                    if matrix500x500[txt_i][txt_j] == 1: 
                        flag = False
                        break
                if(flag == False): break
        ...
        del draw
        img.save('./char/test_' + str(num) + '.jpg')
